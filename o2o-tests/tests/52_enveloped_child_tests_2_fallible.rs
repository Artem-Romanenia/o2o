...
#[derive(o2o)]
#[try_from_owned(Entity, anyhow::Error| vars(child: @.child.unwrap(), base: child.base.unwrap()))]
#[try_from_ref(Entity, anyhow::Error| vars(child: @.child.clone().unwrap(), base: custom_unwrap(child.base)))]
#[try_into(Entity, anyhow::Error)]
#[try_from_owned(TupleEntity as (), anyhow::Error| vars(child: @.1.unwrap(), base: child.1.unwrap()))]
#[try_from_ref(TupleEntity as (), anyhow::Error| vars(child: @.1.clone().unwrap(), base: custom_unwrap_tuple(child.1)))]
#[try_into(TupleEntity as (), anyhow::Error)]
#[child_parents(Entity|
    child: Child => (into: Some(@), from: child), 
    child.base: Base => (into: @.as_ok(), from: base)
)]
#[child_parents(TupleEntity|
    1: TupleChild as () => (into: Some(@), from: child), 
    1 .1: TupleBase as () => (into: @.as_ok(), from: base)
)]
struct EntityDto {
    some_int: i8,
...
#[derive(o2o)]
#[try_from_owned(Entity as {}, anyhow::Error| vars(child: @.child.unwrap(), base: child.base.unwrap()))]
#[try_from_ref(Entity as {}, anyhow::Error| vars(child: @.child.clone().unwrap(), base: custom_unwrap(child.base)))]
#[try_into(Entity as {}, anyhow::Error)]
#[try_from_owned(TupleEntity, anyhow::Error| vars(child: @.1.unwrap(), base: child.1.unwrap()))]
#[try_from_ref(TupleEntity, anyhow::Error| vars(child: @.1.clone().unwrap(), base: custom_unwrap_tuple(child.1)))]
#[try_into(TupleEntity, anyhow::Error)]
#[child_parents(Entity|
    child: Child as {} => (into: Some(@), from: child), 
    child.base: Base as {} => (into: @.as_ok(), from: base)
)]
#[child_parents(TupleEntity|
    1: TupleChild => (into: Some(@), from: child), 
    1 .1: TupleBase => (into: @.as_ok(), from: base)
)]
