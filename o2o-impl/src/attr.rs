...
pub(crate) struct TraitAttrCore {
    ...
    pub match_expr: Option<TokenStream>,
...
fn parse_trait_instruction_param_inner<T: Parse, U>(input: &syn::parse::ParseBuffer, parser: impl Fn(&ParseBuffer) -> Result<U>, condition: bool, setter: impl FnOnce(U), span: impl Fn(T) -> Span, name: &str) -> Result<bool> {
    let a = input.parse::<T>()?;
    let b = parser(input)?;
    if condition {
...
fn parse_trait_instruction_param(input: &syn::parse::ParseBuffer, attr: &mut TraitAttrCore) -> Result<bool> {
    if input.peek(kw::stop_repeat) {
        return parse_trait_instruction_param_inner::<kw::stop_repeat, ()>(input, |_| Ok(()), attr.stop_repeat, |_| attr.stop_repeat = true, |a| a.span, "stop_repeat")
    } else if input.peek(kw::skip_repeat) {
        return parse_trait_instruction_param_inner::<kw::skip_repeat, ()>(input, |_| Ok(()), attr.skip_repeat, |_| attr.skip_repeat = true, |a| a.span, "skip_repeat")
        ...
        return parse_trait_instruction_param_inner::<Token![..], Option<TokenStream>>(input, |x| try_parse_action(x), attr.update.is_some(), |x| attr.update = x, |a| a.span(), "update")
    } else if input.peek(Token![return]) {
        return parse_trait_instruction_param_inner::<Token![return], Option<TokenStream>>(input, |x| try_parse_action(x), attr.quick_return.is_some(), |x| attr.quick_return = x, |a| a.span(), "quick_return")
    } else if input.peek(Token![_]) {
        return parse_trait_instruction_param_inner::<Token![_], Option<TokenStream>>(input, |x| try_parse_action(x), attr.default_case.is_some(), |x| attr.default_case = x, |a| a.span(), "default_case")
    } else if input.peek(Token![match]) {
        return parse_trait_instruction_param_inner::<Token![match], Option<TokenStream>>(input, |x| try_parse_action(x), attr.match_expr.is_some(), |x| attr.match_expr = x, |a| a.span(), "match_expr")
