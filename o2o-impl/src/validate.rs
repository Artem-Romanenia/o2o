use crate::{
    ...
    attr::{
        ChildAttr, ChildrenAttr, DataTypeAttrs, DataTypeInstruction, FallibleKind, GhostsAttr,
        Kind, MemberAttrs, MemberInstruction, TraitAttr, TraitAttrCore, TypeHint, TypePath,
        WhereAttr,
    },
};
...
pub(crate) fn validate(input: &DataType) -> Result<()> {
    ...
    if attrs.attrs.is_empty() {
        errors.insert(
            "At least one trait instruction is expected.".into(),
            Span::call_site(),
        );
    }
    ...
    validate_struct_attrs(
        attrs.iter_for_kind_core(&Kind::FromOwned, false),
        false,
        &mut errors,
    );
    validate_struct_attrs(
        attrs.iter_for_kind_core(&Kind::FromRef, false),
        false,
        &mut errors,
    );
    validate_struct_attrs(
        attrs.iter_for_kind_core(&Kind::OwnedInto, false),
        false,
        &mut errors,
    );
    validate_struct_attrs(
        attrs.iter_for_kind_core(&Kind::RefInto, false),
        false,
        &mut errors,
    );
    validate_struct_attrs(
        attrs.iter_for_kind_core(&Kind::OwnedIntoExisting, false),
        false,
        &mut errors,
    );
    validate_struct_attrs(
        attrs.iter_for_kind_core(&Kind::RefIntoExisting, false),
        false,
        &mut errors,
    );

    validate_struct_attrs(
        attrs.iter_for_kind_core(&Kind::FromOwned, true),
        true,
        &mut errors,
    );
    validate_struct_attrs(
        attrs.iter_for_kind_core(&Kind::FromRef, true),
        true,
        &mut errors,
    );
    validate_struct_attrs(
        attrs.iter_for_kind_core(&Kind::OwnedInto, true),
        true,
        &mut errors,
    );
    validate_struct_attrs(
        attrs.iter_for_kind_core(&Kind::RefInto, true),
        true,
        &mut errors,
    );
    validate_struct_attrs(
        attrs.iter_for_kind_core(&Kind::OwnedIntoExisting, true),
        true,
        &mut errors,
    );
    validate_struct_attrs(
        attrs.iter_for_kind_core(&Kind::RefIntoExisting, true),
        true,
        &mut errors,
    );

    let type_paths = attrs
        .attrs
        .iter()
        .map(|x| &x.core.ty)
        .collect::<HashSet<_>>();

    validate_ghost_attrs(
        &Kind::FromOwned,
        &attrs.ghosts_attrs,
        &type_paths,
        &mut errors,
    ...
    validate_ghost_attrs(
        &Kind::OwnedIntoExisting,
        &attrs.ghosts_attrs,
        &type_paths,
        &mut errors,
    );
    validate_ghost_attrs(
        &Kind::RefIntoExisting,
        &attrs.ghosts_attrs,
        &type_paths,
        &mut errors,
    );
    ...
    for member in input.get_members() {
        ...
        validate_dedicated_member_attrs(
            &member_attrs.attrs,
            |x| x.attr.container_ty.as_ref(),
            None,
            member_span,
            &type_paths,
            &mut errors,
        ...
        match member {
            DataTypeMember::Field(f) => {
                bark_at_member_attr(
                    &member_attrs.lit_attrs,
                    "literal",
                    |_| f.member.span(),
                    &mut errors,
                );
                bark_at_member_attr(
                    &member_attrs.pat_attrs,
                    "pattern",
                    |_| f.member.span(),
                    &mut errors,
                );
                bark_at_member_attr(
                    &member_attrs.type_hint_attrs,
                    "type_hint",
                    |_| f.member.span(),
                    &mut errors,
                );
                bark_at_member_attr(
                    &member_attrs
                        ...
                        .iter()
                        .filter(|x| {
                            x.applicable_to[&Kind::OwnedInto] && x.applicable_to[&Kind::RefInto]
                        })
                        .collect(),
                    "ghosts",
                    |_| f.member.span(),
                ...
                validate_dedicated_member_attrs(
                    &member_attrs.parent_attrs,
                    |x| x.container_ty.as_ref(),
                    Some("parent"),
                    member_span,
                    &type_paths,
                    &mut errors,
    ...
    if errors.is_empty() {
        ...
        errors
            .iter()
            .for_each(|(err, sp)| root_err.combine(syn::Error::new(*sp, err)));

        Err(root_err)
    }
}

fn validate_error_instrs(
    input: &DataType,
    attrs: &DataTypeAttrs,
    errors: &mut HashMap<String, Span>,
) {
    ...
    for err in &attrs.error_instrs {
        match (input, err) {
            ...
            | (
                DataType::Enum(_),
                DataTypeInstruction::Misplaced {
                    instr: instr @ ("parent" | "as_type"),
                    span,
                    own,
                },
            ) => {
                errors.insert(
                    format!(
                        "Member instruction '{}' is not applicable to enums.{}",
                        instr,
                        postfix(*own)
                    ),
                    *span,
            ...
            (
                ...
                errors.insert(
                    format!("Perhaps you meant '{}'?{}", guess_name, postfix(*own)),
                    *span,
                );
            }
            (_, DataTypeInstruction::Misplaced { instr, span, own }) => {
                errors.insert(
                    format!(
                        "Member instruction '{}' should be used on a member.{}",
                        instr,
                        postfix(*own)
                    ),
                    *span,
            ...
            (_, DataTypeInstruction::UnrecognizedWithError { instr, span }) => {
                errors.insert(
                    format!("Struct instruction '{}' is not supported.", instr),
                    *span,
                );
            }
            _ => unreachable!("13"),
...
fn validate_member_error_instrs(
    ...
    for err in &attrs.error_instrs {
        match (input, err) {
            (
                DataType::Enum(_),
                MemberInstruction::Misnamed {
                    instr: instr @ "children",
                    span,
                    guess_name: _,
                    own,
                },
                ...
                errors.insert(
                    format!(
                        "Struct instruction '{}' is not applicable to enums.{}",
                        instr,
                        postfix(*own)
                    ),
                    *span,
            ...
            (_, MemberInstruction::Misplaced { instr, span, own }) => {
                errors.insert(
                    format!(
                        "Struct instruction '{}' should be used on a struct.{}",
                        instr,
                        postfix(*own)
                    ),
                    *span,
            ...
            (_, MemberInstruction::UnrecognizedWithError { instr, span }) => {
                errors.insert(
                    format!("Member instruction '{}' is not supported.", instr),
                    *span,
                );
            }
            _ => unreachable!("14"),
...
fn validate_struct_attrs<'a, I: Iterator<Item = &'a TraitAttrCore>>(
    attrs: I,
    fallible: bool,
    errors: &mut HashMap<String, Span>,
) {
    ...
    for attr in attrs {
        ...
        if fallible && attr.err_ty.is_none() {
            errors.insert(
                "Error type should be specified for fallible instruction.".into(),
                attr.ty.span,
            );
        }
        ...
        if !fallible && attr.err_ty.is_some() {
            errors.insert(
                "Error type should not be specified for infallible instruction.".into(),
                attr.err_ty.as_ref().unwrap().span,
            );
        }
    }
}

fn validate_ghost_attrs(
    kind: &Kind,
    ghost_attrs: &[GhostsAttr],
    type_paths: &HashSet<&TypePath>,
    errors: &mut HashMap<String, Span>,
) {
    if ghost_attrs
        .iter()
        .filter(|x| x.applicable_to[kind] && x.attr.container_ty.is_none())
        .count()
        > 1
    {
        errors.insert(
            "There can be at most one default #[ghosts(...)] instruction.".into(),
            Span::call_site(),
        );
    }
    ...
    for ghost_attr in ghost_attrs
        .iter()
        .filter(|x| x.applicable_to[kind] && x.attr.container_ty.is_some())
    {
        ...
        if !unique_dedicated_attr_type_path.insert(tp) {
            errors.insert(
                format!(
                    "Dedicated #[ghosts(...)] instruction for type {} is already defined.",
                    tp.path_str
                ),
                tp.span,
            );
...
fn validate_children_attrs(
    children_attrs: &[ChildrenAttr],
    type_paths: &HashSet<&TypePath>,
    errors: &mut HashMap<String, Span>,
) {
    if children_attrs
        .iter()
        .filter(|x| x.container_ty.is_none())
        .count()
        > 1
    {
        errors.insert(
            "There can be at most one default #[children(...)] instruction.".into(),
            Span::call_site(),
        );
    }
    ...
    for children_attr in children_attrs.iter() {
        if let Some(tp) = &children_attr.container_ty {
            ...
            if !unique_dedicated_attr_type_path.insert(tp) {
                errors.insert(
                    format!(
                        "Dedicated #[children(...)] instruction for type {} is already defined.",
                        tp.path_str
                    ),
                    tp.span,
                );
        ...
        for child_data in &children_attr.children {
            if !unique_field.insert(child_data) {
                errors.insert(
                    "Ident here must be unique.".into(),
                    child_data.field_path.span(),
                );
            }
        }
    }
...
fn validate_where_attrs(
    where_attrs: &[WhereAttr],
    type_paths: &HashSet<&TypePath>,
    errors: &mut HashMap<String, Span>,
) {
    if where_attrs
    ...
    {
        errors.insert(
            "There can be at most one default #[where_clause(...)] instruction.".into(),
            Span::call_site(),
        );
    }
...
fn bark_at_member_attr<T, U: Fn(&T) -> Span>(
    attrs: &Vec<T>,
...
fn validate_dedicated_member_attrs<T, U: Fn(&T) -> Option<&TypePath>>(
    attrs: &Vec<T>,
    extract_type_path: U,
    instr_name: Option<&'static str>,
    member_span: Span,
    type_paths: &HashSet<&TypePath>,
    errors: &mut HashMap<String, Span>,
) {
    if let Some(inst_name) = instr_name {
        if attrs
            .iter()
            .filter(|x| extract_type_path(x).is_none())
            .count()
            > 1
        {
            errors.insert(
...
fn validate_fields(
    ...
    if !input.named_fields {
        let data_type_attrs: Vec<(&TraitAttrCore, Kind)> = data_type_attrs
            ...
            .chain(
                data_type_attrs
                    .iter_for_kind_core(&Kind::RefInto, false)
                    .map(|x| (x, Kind::RefInto)),
            )
            .chain(
                data_type_attrs
                    .iter_for_kind_core(&Kind::OwnedIntoExisting, false)
                    .map(|x| (x, Kind::OwnedIntoExisting)),
            )
            .chain(
                data_type_attrs
                    .iter_for_kind_core(&Kind::RefIntoExisting, false)
                    .map(|x| (x, Kind::RefIntoExisting)),
            )
            .chain(
                data_type_attrs
                    .iter_for_kind_core(&Kind::FromOwned, false)
                    .map(|x| (x, Kind::FromOwned)),
            )
            .chain(
                data_type_attrs
                    .iter_for_kind_core(&Kind::FromRef, false)
                    .map(|x| (x, Kind::FromRef)),
            )
            .chain(
                data_type_attrs
                    .iter_for_kind_core(&Kind::OwnedInto, true)
                    .map(|x| (x, Kind::OwnedInto)),
            )
            .chain(
                data_type_attrs
                    .iter_for_kind_core(&Kind::RefInto, true)
                    .map(|x| (x, Kind::RefInto)),
            )
            .chain(
                data_type_attrs
                    .iter_for_kind_core(&Kind::OwnedIntoExisting, true)
                    .map(|x| (x, Kind::OwnedIntoExisting)),
            )
            .chain(
                data_type_attrs
                    .iter_for_kind_core(&Kind::RefIntoExisting, true)
                    .map(|x| (x, Kind::RefIntoExisting)),
            )
            .chain(
                data_type_attrs
                    .iter_for_kind_core(&Kind::FromOwned, true)
                    .map(|x| (x, Kind::FromOwned)),
            )
            .chain(
                data_type_attrs
                    .iter_for_kind_core(&Kind::FromRef, true)
                    .map(|x| (x, Kind::FromRef)),
            )
            .collect();
        ...
        for (data_type_attr, kind) in data_type_attrs {
            ...
            {
                for field in &input.fields {
                    if field.attrs.ghost(&data_type_attr.ty, &kind).is_some()
                        || field.attrs.has_parent_attr(&data_type_attr.ty)
                    {
                        continue;
                    }

                    if let Some(field_attr) =
                        field
                            .attrs
                            .applicable_field_attr(&kind, false, &data_type_attr.ty)
                    {
...
fn validate_variant_fields(
    ...
    if !input.named_fields {
        let data_type_attrs: Vec<(&TraitAttr, Kind)> = data_type_attrs
            ...
            .chain(
                data_type_attrs
                    .iter_for_kind(&Kind::RefInto, false)
                    .map(|x| (x, Kind::RefInto)),
            )
            .chain(
                data_type_attrs
                    .iter_for_kind(&Kind::OwnedIntoExisting, false)
                    .map(|x| (x, Kind::OwnedIntoExisting)),
            )
            .chain(
                data_type_attrs
                    .iter_for_kind(&Kind::RefIntoExisting, false)
                    .map(|x| (x, Kind::RefIntoExisting)),
            )
            .chain(
                data_type_attrs
                    .iter_for_kind(&Kind::FromOwned, false)
                    .map(|x| (x, Kind::FromOwned)),
            )
            .chain(
                data_type_attrs
                    .iter_for_kind(&Kind::FromRef, false)
                    .map(|x| (x, Kind::FromRef)),
            )
            .chain(
                data_type_attrs
                    .iter_for_kind(&Kind::OwnedInto, true)
                    .map(|x| (x, Kind::OwnedInto)),
            )
            .chain(
                data_type_attrs
                    .iter_for_kind(&Kind::RefInto, true)
                    .map(|x| (x, Kind::RefInto)),
            )
            .chain(
                data_type_attrs
                    .iter_for_kind(&Kind::OwnedIntoExisting, true)
                    .map(|x| (x, Kind::OwnedIntoExisting)),
            )
            .chain(
                data_type_attrs
                    .iter_for_kind(&Kind::RefIntoExisting, true)
                    .map(|x| (x, Kind::RefIntoExisting)),
            )
            .chain(
                data_type_attrs
                    .iter_for_kind(&Kind::FromOwned, true)
                    .map(|x| (x, Kind::FromOwned)),
            )
            .chain(
                data_type_attrs
                    .iter_for_kind(&Kind::FromRef, true)
                    .map(|x| (x, Kind::FromRef)),
            )
            .collect();

        for (data_type_attr, kind) in data_type_attrs {
            if data_type_attr.core.quick_return.is_none()
                && input
                    .attrs
                    .type_hint(&data_type_attr.core.ty)
                    .map_or(TypeHint::Unspecified, |x| x.type_hint)
                    == TypeHint::Struct
            {
                for field in &input.fields {
                    if field.attrs.ghost(&data_type_attr.core.ty, &kind).is_some()
                        || field.attrs.has_parent_attr(&data_type_attr.core.ty)
                    {
                        continue;
                    }
                    ...
                    if let Some(field_attr) =
                        field
                            .attrs
                            .applicable_field_attr(&kind, false, &data_type_attr.core.ty)
                    {
                        if kind == Kind::FromOwned || kind == Kind::FromRef {
                            if field_attr.attr.member.is_none() && field_attr.attr.action.is_none()
                            {
...
fn check_child_errors(
    child_attr: &ChildAttr,
    struct_attrs: &DataTypeAttrs,
    tp: &TypePath,
    errors: &mut HashMap<String, Span>,
) {
    ...
    for (idx, _level) in child_attr.child_path.child_path.iter().enumerate() {
        ...
        match children_attr {
            ...
            None => {
                errors.insert(
                    format!("Missing #[children(...)] instruction for {}", tp.path_str),
                    tp.span,
                );
            }
        }
