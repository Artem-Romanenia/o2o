...
#[derive(Default)]
#[derive(o2o)]
#[o2o(from(UnnamedStructDto))]
struct NamedStruct {
    #[o2o(from(|x|x.0))] some_int: i32,
    #[o2o(from(|x|x.1))] another_int: i32,
    #[o2o(from(|x|x.2))] some_float: f32,
}
...
struct NamedStructDto {
    some_int: i32,
    #[o2o(
        from(NamedStruct| |x|x.another_int as i16),
        into(NamedStruct| |x|x.another_int as i32),
        from(NamedStructModel| |x|x.another_int as i16),
        into(NamedStructModel| |x|x.another_int as i8),
    )]
    another_int: i16,
    #[o2o(from(|x|x.some_float as f64))]
    #[o2o(into(|x|x.some_float as f32))]
    some_float: f64,
}
...
struct ParentDto2 {
    parent_int: i32,
    #[from(|x|x.child.child_int)] child_int: i32,
    #[from(|x|x.child.another_child_int)] another_child_int: i32,
}
...
struct PersonDto {
    #[map(|x|x.name.clone())]
    name: String,
    #[map(|x|x.pets.iter().map(|p|p.into()).collect())]
    pets: Vec<PetDto>,
}
