...
- **Feature-matrix testing is REQUIRED.** The repo builds against two mutually exclusive `syn` versions and both `std`/`no_std` re-export configurations. You MUST run [tests.sh](tests.sh) (or the equivalent individual commands below) before considering any change complete:
  ```bash
  cargo test -q -p o2o
